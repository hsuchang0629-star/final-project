...
ntListener("click", () => {
  ...
  function calculateRoutePlanner() {
    ...
    if (start === "gps") {
      ...
      // If testing off-site (distance > 1500 meters), mock a 240-meter campus walk
      if (distance > 1500) {
        displayDistance = 240; 
        isClamped = true;
      }

      walkTime = Math.max(1, Math.round(displayDistance / 80)); // walking at 80m/min
      const totalTime = walkTime + waitTime;
      ...
      routePathSteps.innerHTML = `
        <span>GPS 定位位置</span>
        <span class="material-symbols-outlined text-outline-variant text-[20px]">arrow_forward</span>
        <span class="text-primary font-bold">${end} (約 ${Math.round(displayDistance)} 公尺)</span>
      `;

      // Render Visual map (GPS mode has 2 main nodes)
      renderVisualGpsMap(end, displayDistance);

      // Save GPS status text
      gpsRouteStatus.classList.remove("hidden");
      if (isClamped) {
        gpsRouteStatus.className = "text-xs font-semibold pl-2 mt-1 text-amber-600";
        gpsRouteStatus.textContent = `📍 定位成功(模擬校內距離): 距離 ${end} 約 ${displayDistance} 公尺。 (偵測到您在校外)`;
      } else {
        gpsRouteStatus.className = "text-xs font-semibold pl-2 mt-1 text-green-600";
        gpsRouteStatus.textContent = `📍 定位成功：距離 ${end} 約 ${Math.round(distance)} 公尺。`;
      }
    ...
    restaurants.forEach(otherCanteen => {
      ...
      if (start === "gps") {
        const d = calculateHaversineDistance(userCoords.lat, userCoords.lng, otherCanteen.lat, otherCanteen.lng);
        const dispD = d > 1500 ? 240 : d;
        otherWalk = Math.max(1, Math.round(dispD / 80));
      } else {
        const otherPath = runDijkstra(graph, start, "學生餐廳");
        otherWalk = otherPath.distance;
      }
